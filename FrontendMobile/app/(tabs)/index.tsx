...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, ActivityIndicator, Alert, TextInput, Modal, ScrollView, Image, RefreshControl } from 'react-native';
import { useAuth } from '../../context/AuthContext';
import { useRouter } from 'expo-router';
import { Users, Bell, LogOut, Cloud, User, UserPlus, Check, Plus, ShieldAlert, Key } from 'lucide-react-native';
...
export default function ChatsScreen() {
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.pickerContainer, { maxHeight: '90%', padding: 20 }]}>
          ...
          <ScrollView style={{ width: '100%' }} contentContainerStyle={{ alignItems: 'center' }} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ width: '100%', marginTop: 20, borderTopWidth: 1, borderTopColor: 'rgba(255,204,0,0.2)', paddingTop: 15 }}>
              <Text style={{ color: '#ffcc00', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase', marginBottom: 10 }}>Recovery Phrase</Text>
              <TouchableOpacity
                style={{ width: '100%', padding: 14, backgroundColor: 'rgba(255,204,0,0.1)', borderRadius: 10, borderWidth: 1, borderColor: 'rgba(255,204,0,0.3)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                onPress={async () => {
                  const phrase = await SecureStore.getItemAsync(`prama_mnemonic_${(user?.username || '').toLowerCase()}`);
                  if (phrase) {
                    const words = phrase.split(' ');
                    const formatted = words.map((w, i) => `${i + 1}. ${w}`).join('  |  ');
                    Alert.alert('Your Recovery Phrase', formatted, [{ text: 'OK' }]);
                  } else {
                    Alert.alert('Not Found', 'Recovery phrase not found on this device. Register on web to generate one, or use the Recovery screen if you already have your 12 words.');
                  }
                }}
              >
                <Key color="#ffcc00" size={16} />
                <Text style={{ color: '#ffcc00', fontWeight: 'bold', fontSize: 14 }}>View Recovery Phrase</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
  ...
  useEffect(() => {
    ...
    // 🚀 Silent Background Polling: Matches Web app behavior to instantly receive requests
    const pollInterval = setInterval(() => {
      fetchAllSocialData(true);
    }, 10000);
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    fetchAllSocialData(false).finally(() => setRefreshing(false));
  }, []);

  const fetchAllSocialData = async (silent = false) => {
    try {
  ...
  const acceptFriendRequest = async (requestId: string) => {
    try {
      const res = await apiFetch(`${API_BASE_URL}/api/v1/friends/accept/${requestId}`, { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList 
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#66fcf1" colors={['#66fcf1']} progressBackgroundColor="#1f2833" />}
