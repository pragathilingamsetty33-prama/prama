...
export default function LoginScreen() {
  ...
  const [isNewDeviceAuth, setIsNewDeviceAuth] = useState(false);
  const { login, resetIdentity, logout, authorizeNewDevice } = useAuth();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (e.message === 'CRYPTOGRAPHIC_INTEGRITY_ERROR') {
        ...
        setIsNewDeviceAuth(false);
        setShowRecoveryModal(true);
      } else if (e.message === 'NEW_DEVICE_DETECTED') {
        addLog("📱 New Device Detected! Prompting for 12-word authorization.");
        setIsNewDeviceAuth(true);
        setShowRecoveryModal(true);
      } else {
  ...
  const handleRecoveryVerify = async () => {
    ...
    try {
      if (isNewDeviceAuth) {
        addLog("🔑 [Recovery] Authorizing new device...");
        await authorizeNewDevice();
        addLog("🔑 [Recovery] New device authorized successfully!");
          ...
          <View style={styles.recoveryContainer}>
            ...
            <ScrollView style={{ flexGrow: 0, maxHeight: 300 }}>
              <Text style={styles.recoveryWarningText}>
                {isNewDeviceAuth 
                  ? "You are logging in from a new device! To guarantee E2EE integrity, please authorize this device by entering your 12-word recovery phrase."
