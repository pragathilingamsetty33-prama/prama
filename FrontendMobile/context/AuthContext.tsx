...
interface AuthContextType {
  ...
  authorizeNewDevice: () => Promise<void>;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [pendingDeviceSync, setPendingDeviceSync] = useState<any>(null);
          ...
          if (storedUser && sessionPwd) {
            ...
            while (targetVer > 0) {
              try {
                ...
                const encVal = await AsyncStorage.getItem(`rsaKeys_${parsedUser.userId}_v${targetVer}`);
                if (encVal) {
            ...
            if (!storedEncryptedKeys) {
              try {
                ...
                const legacyVal = await AsyncStorage.getItem(`rsaKeys_${parsedUser.userId}`);
                if (legacyVal) {
                  derivedKey = await deriveKeyFromPassword(sessionPwd, parsedUser.userId);
  ...
  const login = async (identifier: string, password: string, onStatusUpdate?: (msg: string) => void) => {
    ...
    while (targetVer > 0) {
      try {
        ...
        const encVal = await AsyncStorage.getItem(`rsaKeys_${data.userId}_v${targetVer}`);
        if (encVal) {
          decryptedStr = decryptDataWithPassword(JSON.parse(encVal), derivedKey);
          storedEncryptedKeys = encVal;
          console.log(`🔑 [AuthContext] Successfully decrypted E2EE key bundle from versioned slot: v${targetVer}`);
    ...
    if (!storedEncryptedKeys) {
      try {
        ...
        const legacyVal = await AsyncStorage.getItem(`rsaKeys_${data.userId}`);
        if (legacyVal) {
          decryptedStr = decryptDataWithPassword(JSON.parse(legacyVal), derivedKey);
          storedEncryptedKeys = legacyVal;
          console.log(`🔑 [AuthContext] Successfully decrypted legacy E2EE key bundle.`);
    ...
    if (!currentKeys) {
      try {
        ...
        if (bundleRes.ok) {
          ...
          console.log('🔑 [AuthContext] Successfully retrieved wrapped key from backend.');
          const bundleData = await bundleRes.json();
          
          if (!localKeysFound) {
            console.log('🚨 [AuthContext] New device detected. Halting for 12-word authorization gate.');
            setPendingDeviceSync({ bundleData, derivedKey, data, password });
        ...
        if (e.message === 'CRYPTOGRAPHIC_INTEGRITY_ERROR' || e.message === 'SERVER_SYNC_ERROR' || e.message === 'NEW_DEVICE_DETECTED') {
    ...
    await AsyncStorage.setItem(`rsaKeys_${data.userId}_v${nextVer}`, JSON.stringify(encryptedKeys));
    await AsyncStorage.setItem(`rsaKeys_${data.userId}`, JSON.stringify(encryptedKeys)); // legacy slot sync for backwards compatibility
  ...
  const logout = async () => {
    ...
    setMasterKey(null);
    if (user) {
      await AsyncStorage.removeItem(`rsaKeys_${user.userId}`);
    }
  ...
  const resetIdentity = async (password: string) => {
    ...
    const nextVer = writeVerStr ? parseInt(writeVerStr, 10) + 1 : 1;
    await AsyncStorage.setItem(`rsaKeys_${currentUser.userId}_v${nextVer}`, JSON.stringify(encryptedKeys));
    await AsyncStorage.setItem(`rsaKeys_${currentUser.userId}`, JSON.stringify(encryptedKeys));
  ...
  const authorizeNewDevice = async () => {
    if (!pendingDeviceSync) throw new Error("No pending device sync context.");
    const { bundleData, derivedKey, data, password } = pendingDeviceSync;

    console.log('🔑 [AuthContext] Authorizing new device and decrypting server bundle...');
    let currentKeys;
    try {
      const decryptedStr = decryptDataWithPassword(bundleData, derivedKey);
      currentKeys = JSON.parse(decryptedStr);
    ...
    console.log('🔑 [AuthContext] Saving secure session to new device...');
    const encryptedKeys = encryptDataWithPassword(JSON.stringify(currentKeys), derivedKey);
    const writeVerStr = await AsyncStorage.getItem(`prama_key_version_${data.userId}`);
    const nextVer = writeVerStr ? parseInt(writeVerStr, 10) + 1 : 1;

    await AsyncStorage.setItem(`rsaKeys_${data.userId}_v${nextVer}`, JSON.stringify(encryptedKeys));
    await AsyncStorage.setItem(`rsaKeys_${data.userId}`, JSON.stringify(encryptedKeys));
    await AsyncStorage.setItem(`prama_key_version_${data.userId}`, String(nextVer));
    ...
    if (currentKeys) {
      try {
        ...
        console.error('❌ [AuthContext] Failed to save keys to SecureStore / SQLite:', e);
      }
    }
    ...
    setMasterKey(derivedKey);
    setUser(data);
    setPendingDeviceSync(null);
    registerPushNotifications(data.accessToken).catch(err => console.warn("FCM dynamic login reg failed:", err));
  };

  return (
    <AuthContext.Provider value={{ user, keys, masterKey, login, register, logout, loading, apiFetch, syncKeysToServer, getOrRefreshToken, resetIdentity, updateProfile, updateAvatar, authorizeNewDevice }}>
      {children}
